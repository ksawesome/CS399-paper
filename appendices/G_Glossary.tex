...
\item[\textbf{Context Fidelity}] An evaluation dimension defined in \cref{sec:evaluation_criteria} to measure how accurately an AI model's response reflects the specific details, constraints, and requirements presented in the user's prompt. High context fidelity indicates that the model attended carefully to contextual nuances rather than generating generic or hallucinated content. Operationalized with a 5-point rubric in \cref{app:context_fidelity_rubric}.
...
\item[\textbf{Inter-Rater Reliability (IRR)}] A statistical measure quantifying the consistency of judgments across multiple human raters evaluating the same items. High IRR (e.g., Cronbach's $\alpha > 0.8$) indicates that raters applied evaluation criteria consistently, lending credibility to aggregated scores. We computed IRR for Socratic Quality and Context Fidelity rubrics using Cronbach's alpha and intra-class correlation (ICC), as detailed in \cref{app:socratic_quality_rubric,app:context_fidelity_rubric}.
...
\item[\textbf{Pedagogical Coherence}] An evaluation dimension defined in \cref{sec:evaluation_criteria} to assess whether an AI tutor's response maintains a logically structured, goal-oriented, and pedagogically sound progression toward learning objectives. High pedagogical coherence indicates that the response builds conceptual scaffolds, sequences explanations appropriately, and avoids tangential or disjointed content. Operationalized with a 5-point rubric as part of the Socratic Quality assessment.
...
\item[\textbf{Socratic Quality}] A composite evaluation dimension defined in \cref{sec:evaluation_criteria} to measure the extent to which an AI tutor's response embodies principles of Socratic pedagogy. Socratic Quality aggregates five subdimensions: \textit{Question Framing} (clarity and relevance of questions posed), \textit{Pedagogical Coherence} (logical progression toward learning objectives), \textit{Conceptual Depth} (engagement with underlying principles rather than surface-level facts), \textit{Student Autonomy} (encouragement of independent reasoning), and \textit{Adaptive Scaffolding} (responsiveness to implied student knowledge level). Operationalized with detailed rubrics in \cref{app:socratic_quality_rubric}.
...
\paragraph{Further reading.} For readers seeking deeper technical background on large language models and their applications in education, we recommend:
\begin{itemize}
\item \textbf{Foundational transformer paper}: Vaswani et al., "Attention Is All You Need," \textit{NeurIPS} 2017.
\item \textbf{GPT architecture}: Brown et al., "Language Models are Few-Shot Learners," \textit{NeurIPS} 2020.
\item \textbf{AI in education survey}: Holmes et al., "Artificial Intelligence In Education," \textit{Nature Reviews Education} 2022.
\item \textbf{Socratic pedagogy}: Paul, R., \& Elder, L., "The Art of Socratic Questioning," \textit{Foundation for Critical Thinking} 2007.
\end{itemize}

These resources provide comprehensive overviews of the technical and pedagogical foundations underpinning this study.
