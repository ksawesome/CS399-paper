...
The harness reads provider credentials from a local \verb|.env| file that is not version-controlled. Logs exclude secrets, and results contain only synthetic prompts and model outputs. No personally identifiable information (PII) is collected; any safety screening is performed on synthetic content. Provider-side data retention remains disabled when the API supports it. These controls align with the safety posture described in \cref{subsec:safety_ethics}.
...
\paragraph{Derived features.} During ingestion, we compute secondary features including response length (characters/tokens), interrogative-to-declarative ratio (a proxy for Socratic style), and keyword coverage. These features support automated analysis and stratified sampling for human review in \cref{subsec:human_evaluation}.
...
\paragraph{Provenance and sources.} All results in this paper are derived from these harness-generated artifacts and the associated human-scoring exports described in \cref{subsubsec:evaluation_interface,subsubsec:evaluation_protocol}. No external human data (e.g., classroom transcripts) were used; all prompts are synthetic and curated for this study (\cref{subsec:datasets_prompt_bank}). Each human rating is keyed back to the originating harness record via the stable identifiers (e.g., \verb|prompt_id|, model, run timestamp), ensuring a verifiable chain from prompt to score.
